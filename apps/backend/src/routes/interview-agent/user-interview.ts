...
router.use(
    ...
    asyncHandler<ProtectedRequest>(async (_req, res) => {
        const availableInterviews =
            await interviewAgentRepo.getAvailableInterviews();

        new SuccessResponse(
            'Fetched available interviews.',
            availableInterviews.map((availableInterview) => ({
                ..._.omit(availableInterview, ['createdBy']),
                ...{
                    hiringManagerName: availableInterview.createdBy.name,
                    companyName:
                        availableInterview.createdBy.hiringManagerInformation
                            ?.companyName,
                },
            })),
        ).send(res);
